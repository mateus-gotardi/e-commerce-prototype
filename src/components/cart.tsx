...
export default function Cart({ close }: { close: () => void }) {
    ...
    const textNormal = "md:text-lg text-sm text-secondary text-center"
    ...
    return (
        <div className={cn("absolute z-50 top-20 sm:left-[5vw] left-2 bg-accents md:px-10 px-3 md:pb-10 pt-2 pb-7 w-[90vw] rounded-2xl shadow-2xl")}>
            ...
            <div className={cn("flex justify-between items-center md:flex-row flex-col md:mb-0 mb-3")}>
            ...
            <div className={cn("flex flex-col gap-3")}>
                {cartItems.map((item, key) => {
                    return (
                        <div key={key} className={cn("flex flex-col md:flex-row w-full justify-between items-center bg-accents-2 px-10 py-2 rounded-2xl")}>
                            ...
                            <div className={cn("flex min-w-36 w-fit gap-3 justify-between items-center bg-accents px-2 py-2 rounded-2xl")}>
            ...
            <div className={cn("flex justify-between items-center mt-10")}>
                {cartItems.length === 0 ? <h2 className={cn(subsubtitle, "sm:text-left text-center w-full")}>Your cart is empty</h2> : <>
                    <h2 className={cn(subsubtitle, "text-base sm:text-xl")}>Total: <span className={cn(underlined)}>{`${getTotal()} K`}</span></h2>
